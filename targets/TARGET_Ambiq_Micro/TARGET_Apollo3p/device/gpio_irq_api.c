...
{
 	AM_HAL_GPIO_MASKCREATE(gpio_int_mask);
 	am_hal_gpio_interrupt_status_get(true, pgpio_int_mask);
 	am_hal_gpio_interrupt_clear(pgpio_int_mask);
  	am_hal_gpio_interrupt_service(pgpio_int_mask);
}
//-------------------------------------------------
//original below
/*    void am_gpio_isr(void)
 ...
 */
    ...
    {
        AM_HAL_GPIO_MASKCREATE(gpio_int_mask);
        AM_HAL_GPIO_MASKBIT(pgpio_int_mask, obj->control->pad);
        am_hal_gpio_interrupt_clear(pgpio_int_mask);
        am_hal_gpio_interrupt_enable(pgpio_int_mask);
        
        //am_hal_gpio_interrupt_clear(AM_HAL_GPIO_BIT(obj->control->pad)); //apollo3 method?
 ...
 */
    void gpio_irq_disable(gpio_irq_t *obj)
    {
        ...
        AM_HAL_GPIO_MASKBIT(pgpio_int_mask, obj->control->pad);
        am_hal_gpio_interrupt_clear(pgpio_int_mask);
        am_hal_gpio_interrupt_disable(pgpio_int_mask);
